  ...
  if(isset($_POST['update_booking']))
    {
            ...
            $query="update tms_appointment_bookings set Full_Name=?, Email=?, Phone_no=?, Car_Model=?, service_type=?, Date=?, Time=?, Special_Req=? where u_id=?";
            $stmt = $mysqli->prepare($query);
            $rc=$stmt->bind_param('sssssssss',$Full_Name, $Email, $Phone_no, $Car_Model, $s_type, $Date, $Time, $Special_Req, $u_id);
            $stmt->execute();
                if($stmt)
                {
